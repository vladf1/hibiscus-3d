...
The maintained web viewer is ../index.html and ../src/viewer.js, built by the repository's root Vite configuration. The web model is ../assets/hibiscus.glb. `npm run optimize:model` builds it from hibiscus.glb here and also applies ../scripts/surface-details.mjs, which seats the dew droplets on the petal fronts and textures the stems and calyx. Those passes are not in hibiscus.blend. Working files stay in the repository; only viewer dependencies are included in the Pages build.
